...
class RpcPeerTest {
    ...
    fun `a call is answered by the far side`() = runBlocking {
        coroutineScope {
            ...
            val result = caller.call(NodeId.SERVER, InvokeProcedure("run", ProcedureId("b"), NodeId.SERVER))
    ...
    fun `a failure on the far side arrives with its type and message intact`() = runBlocking {
        coroutineScope {
            val caller = RpcPeer(hub.connect(NodeId.ORCHESTRATOR))
            val callee = RpcPeer(hub.connect(NodeId.SERVER))
            callee.onRequest = { throw AssertionFailure("the block was not there") }
            val jobs = listOf(caller.start(this), callee.start(this))

            val thrown = assertThrows<RemoteInvocationException> {
                caller.call(NodeId.SERVER, InvokeProcedure("run", ProcedureId("b"), NodeId.SERVER))
            }
    ...
    fun `a node can call back into the caller while the caller is still waiting`() = runBlocking {
        coroutineScope {
            ...
            server.onRequest = {
                ...
                orchestrator.call(NodeId.ORCHESTRATOR, InvokeProcedure("run", ProcedureId("nested"), NodeId.ORCHESTRATOR))
            }
            val jobs = listOf(orchestrator.start(this), server.start(this))

            val result = orchestrator.call(NodeId.SERVER, InvokeProcedure("run", ProcedureId("b"), NodeId.SERVER))
    ...
    fun `envelopes survive being encoded and decoded`() = runBlocking {
        coroutineScope {
            ...
            val sent = InvokeProcedure("run", ProcedureId("b"), NodeId.SERVER, "a test", listOf(JsonPrimitive(3)))
