...
internal class RpcFirExtensionRegistrar(private val roles: RoleIndex) : FirExtensionRegistrar() {
    override fun ExtensionRegistrarContext.configurePlugin() {
        // A lambda rather than a constructor reference, because the checker needs the index
        // as well as the session.
        +{ session: FirSession -> RpcCheckersExtension(session, roles) }
    }
}
