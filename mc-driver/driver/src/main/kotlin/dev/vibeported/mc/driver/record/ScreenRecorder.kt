...
import dev.vibeported.mc.driver.RecordingOptions
import dev.vibeported.mc.driver.VideoCodec
import net.minecraft.client.Minecraft
import dev.vibeported.mc.driver.Capture
import java.io.File
...
public object ScreenRecorder {
     ...
     */
    public fun start(client: String, videoFileName: String, options: RecordingOptions): File? {
        stop()
    ...
    /**
     * Where a recording goes: `<capture dir>/recordings/<client>/<name>`.
     *
     * The name the caller gave is used as it wrote it, with an `.mp4` added when it did not, so a
     * file is findable by the name at the call site rather than by a mangling of it.
     */
    public fun fileFor(client: String, videoFileName: String): File? {
        val directory = Capture.directory("recordings", client) ?: return null
        directory.mkdirs()
        return File(directory, Capture.sanitise(videoFileName.removeSuffix(".mp4")) + ".mp4")
    }
    ...
    private fun stopBecause(reason: String) {
        ...
        refusal = reason
        println("mcdriver: screen recording stopped -- $reason")
    }
    ...
    private class Session(private val file: File, private val options: RecordingOptions) {
        ...
        private fun open(glTexture: Int, targetWidth: Int, targetHeight: Int) {
            ...
            worker = Thread({ encodeLoop() }, "mcdriver-screen-recorder").apply {
                isDaemon = true
                start()
            }

            println(
                "mcdriver: recording ${targetWidth}x$targetHeight at ${options.fps} fps with " +
        ...
        private fun encodeLoop() {
            ...
            try {
                ...
                println("mcdriver: screen recording failed -- $failure")
            } finally {
        ...
        fun finish(): File {
            ...
            if (encoded.get() == 0 && worker == null) {
                ...
                println("mcdriver: nothing was recorded for ${file.name}")
                return file
            }

            val lost = dropped.get()
            println(
                "mcdriver: recorded ${encoded.get()} frames into ${file.absolutePath}" +
