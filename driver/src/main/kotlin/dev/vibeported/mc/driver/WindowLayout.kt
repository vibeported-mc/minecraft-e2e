 ...
 * other, and a run of two becomes a run of one you can watch. Whatever launched the games knows how
 * many there are and which this is; the monitor's size is only knowable here, so the split is
 * ordinal from there, arithmetic here.
 */
internal object WindowLayout {

    private const val INDEX_PROPERTY = "mcdriver.window.index"
    private const val COUNT_PROPERTY = "mcdriver.window.count"
