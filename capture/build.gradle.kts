...
// 25, against the 21 the rest of the tree builds on: the FFM API these bindings are made
// of only became final in 22, and on 21 it is a preview API that will not compile without
// flags. This runs after the root build script has had its say, so it wins.
subprojects {
    apply(plugin = "java")
...
// Where the two halves meet. The subprojects read these rather than reaching into build/
// by hand -- and they read them off this project, not the root, which is minecraft-e2e.
