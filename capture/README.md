...
Three modules of the `minecraft-e2e` build, and no dependency on Minecraft in any
of them. They hold a reproducible cross-build of the FFmpeg libraries for Windows
cut down to what capture needs, Panama bindings generated from those exact
headers, an object-oriented layer over them, and an example that records itself.
...
Run from the root of `minecraft-e2e`, like any other module:

```powershell
.\gradlew :capture:libav:build       # the bindings, and the DLLs they wrap
.\gradlew :capture:example:run       # renders a triangle, writes example\out.mp4
```
...
in well under a second. Only a clean, or an edit under `docker/`, brings docker
back into the loop.
...
Three modules, all built by an ordinary `gradlew build` of the tree:
